RSpec.describe "Ruby's Hash structure" do
  ...
  it "can accept default argument to #fetch" do
    hash = Hash.new

    expect( hash.fetch(:z, "foo bar") ).to eq("foo bar")
  end

  it "can accept a block to #fetch" do
    hash = Hash.new
    hash[:a] = "Hello, a"

    expect( hash.fetch(:a) { |el| "Hello, #{el}" } ).to eq("Hello, a")
    expect( hash.fetch(:z) { |el| "Goodbye, #{el}" } ).to eq("Goodbye, z")
  end
end
